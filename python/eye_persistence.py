# -*- coding: utf-8 -*-
"""Port of matlab_src/func/func_eyediag.m (numeric part only).

Computes the persistence matrix N (hvec x OSRup-phases) the way MATLAB
does: chunked >linear interpolation, per-phase histogram accumulation.
Plotting (style-free) is done by plots.py.
"""
...
def eye_persistence(data, settle_ui, delay, OSR, data_rate, Tsym,
                    saturation=1.0):
    data4eye = np.asarray(data, dtype=np.float64)[settle_ui * OSR:]
