# -*- coding: utf-8 -*-
"""Pulse-response fitting, equalization, and transfer-function analysis.
...
class PulseResponseTool:
    ...
    def _constellation_sndr_helper_f(self, dfe_status, dfe_external=None):
        ...
        for zz in range(1, uiSample + 1):
            ...
            if sndr[zz - 1] > const_sndr:
                ...
                best_phase_found = True

        if not best_phase_found:
            raise RuntimeError(
                'No valid constellation phase found: all per-phase SNDR '
                'values were non-positive or non-finite')
    ...
    def calc_tf_from_linear_fit_data_f(self, rise_fall_time=1e-15):
        ...
        tf = tf / tf[0]
        # NOTE: MATLAB original uses freq = [1:len(p)]/len(p)*UISAMPLES*DATARATE
        # which mis-pairs the 1-based index with the FFT bin (fft bin #1 is DC,
        # 0 Hz) — a genuine off-by-one in the source.  We use the physically
        # correct 0-based frequency axis: freq[k] = k/N * Fs  (k = 0 .. N-1),
        # so freq[0] = 0 Hz == DC bin and freq[N/2] = symbol_rate/2 (Nyquist).
        freq = np.arange(len(p)) / len(p) * UISAMPLES * DATARATE
