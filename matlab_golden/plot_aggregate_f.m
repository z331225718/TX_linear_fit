function plot_aggregate_f(input_files_vec,num_of_row,num_of_col,output_path,options)
    arguments
        input_files_vec
        num_of_row
        num_of_col
    ...
    for idx_row = 1:num_of_row
        for idx_col = 1:num_of_col
            ...
            % GOLDEN FIX: original called non-existent load_adjust_fig_v2_f;
            % MATLAB dispatches by FILE NAME -> load_adjust_fig_v2
            sfh_temp = load_adjust_fig_v2(input_files_vec{i},sfh_temp,xsize,xshift(idx_col),row_yshift(idx_row),ysize_max);
            i = i+1;
        end
    end
