function [results] = lab_tx_linear_fit_f(options)
arguments
...
% GOLDEN FIX: original call was process_input_f(options.filetype, options.filename)
% which binds positionally to lab_txt_data_filename / lab_csv_data_filename.
% Intent: select branch by filetype.
switch options.filetype
    case 'lab_txt'
        [lab_data] = process_input_f('lab_txt_data_filename', options.filename);
    case 'lab_csv'
        [lab_data] = process_input_f('lab_csv_data_filename', options.filename);
    otherwise
        error('lab_tx_linear_fit_f: unknown filetype "%s"', options.filetype);
end
...
% GOLDEN FIX: expose everything for capture
results.prt = prt;
end
