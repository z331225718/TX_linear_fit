    ...
    methods
        ...
        function [] = plot_linear_fit_pulse_f(obj,options)
            ...
            function [] = plot_eye_diagram(obj,data_vec,eye_config,options)
                ...
                ylabel('Voltage(V)');
                xlabel('Time(s)');
                % GOLDEN FIX: export name must match plot_aggregate_f's list
                if contains(options.title,'Unequalized')
                    eye_fname = 'eye_Unequalized.png';
                else
                    eye_fname = 'eye_Equalized.png';
                end
                exportgraphics(gca,['results/' eye_fname],'Resolution',500);
            end
